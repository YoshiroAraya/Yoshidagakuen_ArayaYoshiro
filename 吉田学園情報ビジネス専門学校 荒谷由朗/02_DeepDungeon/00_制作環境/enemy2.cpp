...
float fJump_E_2;	//ジャンプ力
float fGRAVITY_E_E_2;	//重力
...
void InitEnemy_2(void)
{
	...
	for (int nCntEnemy = 0; nCntEnemy < MAX_ENEMY_2; nCntEnemy++)
	{
		...
		g_Enemy_2[nCntEnemy].fAngle = 0.0f;			//差分
...
void UpdateEnemy_2(void)
{
	...
	EnemyMotion_2 *pE_Motion = GetEnemyMotion_2();					//モーションデータの所得
	EnemyTex_2 *pE_Text = GetEnemyTex_2();							//ファイルデータの取得
	...
	for (int nCntEnemy = 0; nCntEnemy < MAX_ENEMY_2; nCntEnemy++)
	{
		...
		if (g_Enemy_2[nCntEnemy].bUse == true)
		{
			...
			g_Enemy_2[nCntEnemy].fLength = sqrtf((pPlayer->pos.x - g_Enemy_2[nCntEnemy].pos.x) * (pPlayer->pos.x - g_Enemy_2[nCntEnemy].pos.x) + (pPlayer->pos.z - g_Enemy_2[nCntEnemy].pos.z) *(pPlayer->pos.z - g_Enemy_2[nCntEnemy].pos.z));//距離
...
#endif
			SetPositionSencerCollision(g_Enemy_2[nCntEnemy].nIndexSencer, g_Enemy_2[nCntEnemy].pos);//センサーの位置
