...
float fJump_A_0;	//ジャンプ力
float fGRAVITY_E_A_0;	//重力
...
void InitEnemyARCHER_0(void)
{
	...
	for (int nCntEnemy = 0; nCntEnemy < MAX_ENEMYARCHER_0; nCntEnemy++)
	{
		...
		g_EnemyArcher_0[nCntEnemy].fAngle = 0.0f;			//差分
...
void UpdateEnemyARCHER_0(void)
{
	...
	EnemyArcherMotion_0 *pE_Motion = GetEnemyArcherMotion_0();					//モーションデータの所得
	EnemyArcherTex_0 *pE_Text = GetEnemyArcherTex_0();							//ファイルデータの取得
	...
	for (int nCntEnemy = 0; nCntEnemy < MAX_ENEMYARCHER_0; nCntEnemy++)
	{
		...
		if (g_EnemyArcher_0[nCntEnemy].bUse == true)
		{
			...
			g_EnemyArcher_0[nCntEnemy].fLength = sqrtf((pPlayer->pos.x - g_EnemyArcher_0[nCntEnemy].pos.x) * (pPlayer->pos.x - g_EnemyArcher_0[nCntEnemy].pos.x) + (pPlayer->pos.z - g_EnemyArcher_0[nCntEnemy].pos.z) *(pPlayer->pos.z - g_EnemyArcher_0[nCntEnemy].pos.z));//距離
...
#endif
			SetPositionSencerCollision(g_EnemyArcher_0[nCntEnemy].nIndexSencer, g_EnemyArcher_0[nCntEnemy].pos);//センサーの位置
