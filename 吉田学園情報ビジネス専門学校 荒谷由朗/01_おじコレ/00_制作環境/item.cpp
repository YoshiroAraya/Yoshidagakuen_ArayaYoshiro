	...
	for (nCntItem = 0; nCntItem < MAX_ITEM; nCntItem++)
	{
		...
		{// アイテムが使用されている
			...
			if ((g_aItem[nCntItem].nCounterAnim % ITEM_TEXTURE_SPEED) == 0)		// アニメーションの速さを変えられる
			{
				...
				if (g_aItem[nCntItem].pos.x - g_aItem[nCntItem].fWidth <= g_aItem[nCntItem].posStart.x - g_aItem[nCntItem].fLimit		//左
					|| g_aItem[nCntItem].pos.x + g_aItem[nCntItem].fWidth >= g_aItem[nCntItem].posStart.x + g_aItem[nCntItem].fLimit)	//右
				{// 移動制限(左右)
					g_aItem[nCntItem].move.x *= -1;
				}

				if (g_aItem[nCntItem].pos.x - g_aItem[nCntItem].fWidth <= g_aItem[nCntItem].posStart.x - g_aItem[nCntItem].fLimit)
				{// 左端までいったとき
					g_aItem[nCntItem].nDirectionMove = 0;
				}
				if (g_aItem[nCntItem].pos.x + g_aItem[nCntItem].fWidth >= g_aItem[nCntItem].posStart.x + g_aItem[nCntItem].fLimit)
...
// アイテムの設定処理
//=============================================================================
void SetItem(D3DXVECTOR3 pos, D3DXVECTOR3 move, float fLimit, float fWidth, float fHeight)
{
	int nCntItem;
	...
	for (nCntItem = 0; nCntItem < MAX_ITEM; nCntItem++)
	{
		if (g_aItem[nCntItem].bUse == false)
		{
			...
			g_aItem[nCntItem].fLimit = fLimit;
	...
	for (nCntItem = 0; nCntItem < MAX_ITEM; nCntItem++)
	{
		if (g_aItem[nCntItem].bUse == true)
		{
				...
				&& g_aItem[nCntItem].pos.y + g_aItem[nCntItem].fHeight + fHeight > pPos->y)
			{
				...
				PlaySound(SOUND_LABEL_SE_OZISANGET);	// サウンドの再生
				g_nNumItem--;							// アイテム数の減算
				g_aItem[nCntItem].bUse = false;			// 使用していない状態に
			}
		}
	}
	// 頂点バッファのアンロック
