...
// マクロ定義
//=============================================================================
#define BG_TEXTURENAME_MAX		(3)											//　読み込むテクスチャの最大数
#define BG_TEXTURENAME_WIN		"data/TEXTURE/gameclear_image2.png"			//　読み込むテクスチャ名
#define BG_TEXTURENAME_LOSE		"data/TEXTURE/gameover_image2.png"			//　読み込むテクスチャ名
#define BG_TEXTURENAME_RETRY	"data/TEXTURE/pause001.png"					//　読み込むテクスチャ名
#define BG_TEXTURENAME_QUIT		"data/TEXTURE/pause002.png"					//　読み込むテクスチャ名
#define BG_POS_X				(0)											//　頂点座標（X座標）
#define BG_POS_Y				(0)											//　頂点座標（Y座標）
#define BG_POS_WIDTH			(SCREEN_WIDTH)								//　幅
#define BG_POS_HEIGHT			(SCREEN_HEIGHT)								//　高さ
...
void InitResult(void)
{
	...
	GAMESTATE			pGame	= GetGameStateNow();	// ゲームの取得
	g_nResultNum = 0;									// 選択番号
...
void UpdateResult(void)
{
	...
	if (GetKeyboardTrigger(DIK_LEFT) == true || GetKeyboardTrigger(DIK_A) == true)
	{
		...
		g_nResultNum = (g_nResultNum + 1) % 2;	// 選択番号の切り替え
		g_fResultCol = 1.0f;					// 透明度のリセット
