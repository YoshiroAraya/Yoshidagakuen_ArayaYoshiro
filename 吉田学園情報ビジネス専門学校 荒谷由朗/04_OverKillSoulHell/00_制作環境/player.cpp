...
CPlayer::CPlayer():CScene2D(3)
{
	...
	m_mode = CManager::GetMode();	// モードの取得
...
void CPlayer::Update(void)
{
				...
				state.Gamepad.wButtons & XINPUT_GAMEPAD_A && abConect[0] == true)
			{
				...
				case PLAYER_BULLETTYPE_SLASH:
					if (m_nCount > 20)
					{
						...
						m_nCount = 0;
						pSound->PlaySound(CSound::SOUND_LABEL_PLAYER_BULLET);	// サウンドの再生
					}
					break;
					...
					if (m_nSkillCount > 20)
					{
						if (nSkillNum == 0)
						{
							if (m_Mp >= 1)
							{
								...
								pSound->PlaySound(CSound::SOUND_LABEL_SKILL);	// サウンドの再生
							}
						}
						else if (nSkillNum == 1)
						{
	...
	if (m_StatusType == PLAYER_STATUSTYPE_DAMAGE)
	{
		...
		if (m_nCntDamage < 30)
		{
			...
			CScene2D::SetCol(D3DXCOLOR(1.0f, 1.0f, 1.0f, m_DamageCol));
		}
		else
		{
			m_nCntDamage = 0;
