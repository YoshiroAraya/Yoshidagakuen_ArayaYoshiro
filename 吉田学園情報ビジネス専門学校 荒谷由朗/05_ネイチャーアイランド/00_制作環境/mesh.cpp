...
HRESULT CMesh::Init(void)
{
	...
	case 1:
		...
		{
			//横のブロック
			for (nCntCorss = 0; nCntCorss < m_nMeshX + 1; nCntCorss++, nCntIdx++)
			{//横軸カウント
				//pIdx[番号] = (指定した横 + １（この１は０からスタートした時に１足りないから） + インデックスの回数)
				pIdx[0] = (m_nMeshX + 1) + nCntIdx;
				pIdx[1] = 0 + nCntIdx;
...
//　メッシュの情報
//===============================================================================
void CMesh::SetMeshInfo(int nMeshX, int nMeshZ, float fMeshDepth, float fMeshWidth)
{
...
bool CMesh::GetHeight(D3DXVECTOR3 &pos)
{
	...
	CDebugProc::Print("現在のブロック　横に %d　ブロック目\n", nMeshX);
	CDebugProc::Print("現在のブロック　縦に %d　ブロック目\n", nMeshZ);
	...
	{//三角形より右にいる場合
		...
		//デバック用
		//CDebugProc::Print("nMeshRU[%d]\n", nMeshRU);
		//CDebugProc::Print("nMeshRD[%d]\n", nMeshRD);
		//CDebugProc::Print("nMeshLU[%d]\n", nMeshLU);
