...
void CEffect::Draw(void)
{
	...
	else if (EffectPos.x + 1000.0f >= CameraPosV.x && EffectPos.x - 1000.0f <= CameraPosV.x && pMode == CManager::MODE_TITLE)
	{//近くのエフェクト以外を消す
		CScene3D::Draw();
	}
	else if(pMode != CManager::MODE_TITLE && pMode != CManager::MODE_GAME)
	{
		CScene3D::Draw();
	}
