...
PLAYER_MOSIONSTATE	g_PLAYER_MOSIONSTATENow	= PLAYER_MOSIONSTATE_NEUTRAL;		//プレイヤー(現在)の状態
...
void InitPlayer(void)
{
	...
	for (int nCntFile = 0; nCntFile < g_Player.MAX_PARTS; nCntFile++)
	{
		...
		g_Player.aModel[nCntFile].pmat = (D3DXMATERIAL*)g_Player.aModel[nCntFile].pBuffMat->GetBufferPointer();
		g_Player.aModel[nCntFile].pTexture = new LPDIRECT3DTEXTURE9[g_Player.aModel[nCntFile].nNumMat];

		for (int nCntMat = 0; nCntMat < (int)g_Player.aModel[nCntFile].nNumMat; nCntMat++)
		{
			if (g_Player.aModel[nCntFile].pmat[nCntMat].pTextureFilename != NULL &&
				lstrlen(g_Player.aModel[nCntFile].pmat[nCntMat].pTextureFilename) > 0)
			{// テクスチャを使用している
				if (FAILED(D3DXCreateTextureFromFile(pDevice,
					g_Player.aModel[nCntFile].pmat[nCntMat].pTextureFilename,
					&g_Player.aModel[nCntFile].pTexture[nCntMat])))
				{
...
void DrawPlayer(void)
{
	...
	for (int nCntParts = 0; nCntParts < g_Player.MAX_PARTS; nCntParts++)
	{
		...
		for (int nCntMat = 0; nCntMat < (int)g_Player.aModel[nCntParts].nNumMat; nCntMat++)
		{
			...
			pDevice->SetTexture(0, *g_Player.aModel[nCntParts].pTexture);
			//モデル(パーツ)の描画
